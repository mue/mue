  ...
  try {
    ...
    const parsed = {
      ...
      photoInfo: {
        ...
        camera: data.camera,
        url: data.file,
        photographerURL: api === 'unsplash' ? data.photographer_page : api === 'mue' && data.photographer ? `https://mue.photos/${data.photographer.toLowerCase().replace(/\s+/g, '-')}` : undefined,
        photoURL: api === 'unsplash' ? data.photo_page : api === 'mue' && data.id ? `https://mue.photos/p/${data.id}` : undefined,
        latitude: data.location.latitude,
        longitude: data.location.longitude,
        views: data.views ?? 0,
        downloads: data.downloads ?? 0,
        likes: data.likes ?? data.hearts ?? 0,
